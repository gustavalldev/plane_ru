    ...
    await updateCurrentUser({
      ...
      .catch(() => {
        setToast({
          ...
          title: "Ошибка",
          message: "Не удалось удалить фото профиля. Попробуйте еще раз.",
        });
      })
      .finally(() => {
  ...
  const onSubmit = async (formData: TUserProfileForm) => {
    ...
    try {
      ...
      setToast({
        type: TOAST_TYPE.ERROR,
        title: t("toast.error"),
        message: error instanceof Error ? error.message : "Не удалось обработать обложку",
      });
      setIsLoading(false);
      return;
    }
    ...
    const updatePromise = Promise.allSettled(promises)
      .then((results) => {
        const rejectedResult = results.find((result) => result.status === "rejected") as
          ...
          | undefined;
        if (rejectedResult) {
          throw rejectedResult.reason ?? new Error("Не удалось обновить профиль");
        }
        ...
        <div className="flex w-full flex-col gap-7">
          <div className="relative h-44 w-full">
            <CoverImage
              ...
              alt={currentUser?.first_name ?? "Обложка"}
            />
